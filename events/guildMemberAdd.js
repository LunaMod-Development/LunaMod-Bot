...
module.exports = async (client, member) => {
  if (require('../database/antibot.json')[member.guild.id]) {
    if (require('../database/antibot.json')[member.guild.id].func === true) {
      let { user_punish, bot_punish } = require('../database/antibot.json')[member.guild.id]
        .settings;

      if (member.user.bot) {
        let userAddedID = await member.guild
          .fetchAuditLogs()
          .then((x) => x.entries.first().executor.id);
        ...
        if (member.manageable) {
          ...
          if (bot_punish === 'ban') {
            member
              .ban({ reason: 'Anti-bot is enabled. LunaMod will now prevent raids!' })
              .catch(() => {});
          }
        }
