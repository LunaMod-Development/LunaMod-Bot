...
module.exports = async (client, message) => {
      ...
      if (require('../../database/perms.json')[command.config.name][message.guild.id] !== 'NONE') {
        if (
          !message.member.hasPermission(
            require('../../database/perms.json')[command.config.name][message.guild.id],
          )
        ) {
          neededPerms = [];
          await neededPerms.push(
            '`' + require('../../database/perms.json')[command.config.name][message.guild.id] + '`',
          );
        }
      } else {
      ...
      try {
        ...
        try {
          ...
          const cantSend = await new MessageEmbed()
            ...
            .setDescription(
              'I am missing permissions to send messages in **' + message.guild.name + '**',
            )
  ...
  if (command.config.ownerOnly) {
    if (command.config.ownerOnly === true) {
      if (!owners.includes(message.author.id))
        return message.reply(`That command is a owner only command LMAO`);
    }
  }
  ...
  if (command.config.botperms) {
    ...
    if (neededPerms.length) {
      const noPEmbe = await new MessageEmbed()
        .setColor(client.color)
        .setDescription(
          `I am missing ${neededPerms.join(
            ', ',
          )} permission(s) give em to me or i will not be able to perform the given tasks ;-;`,
        );
      return await message.channel.send(noPEmbe);
      //  return message.member.send(cantSend);
    }
  ...
  if (commandFile) {
    ...
    try {
      if (
        (await require('../../database/enables.json')[commandFile.config.name][
          message.guild.id
        ]) === true
      ) {
    ...
    if (!message.guild.me.hasPermission('USE_EXTERNAL_EMOJIS'))
      return message.reply('I cannot do anything without `USE_EXTERNAL_EMOJIS` permissions');
